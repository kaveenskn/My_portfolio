...
import { CheckCircle2, Mail, Download, Code2, Database, BrainCircuit, ShieldAlert } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start py-10">
      ...
      <motion.div
        ...
        className={`lg:col-span-5 w-full backdrop-blur-3xl border rounded-[60px] p-10 md:p-14 flex flex-col items-center text-center sticky top-24 transition-all duration-500 ${
          isLight 
          ? "bg-white/70 border-black/5 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)]" 
          : "bg-white/[0.02] border-white/[0.05] shadow-[0_0_50px_-12px_rgba(139,92,246,0.25)]"
        }`}
      >
        <div className="relative w-full aspect-square max-w-[320px] mb-10 group">
          {/* Main Glow behind image */}
          <div className={`absolute -inset-4 rounded-[70px] blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${
            isLight ? "bg-purple-200/50" : "bg-purple-600/20"
          }`} />
          
          <div className={`relative w-full h-full rounded-[60px] overflow-hidden border-2 transition-all duration-700 group-hover:scale-[1.02] ${
            isLight ? "border-purple-500/10 shadow-xl" : "border-white/10 shadow-[0_0_30px_rgba(139,92,246,0.1)]"
          }`}>
            <Image
              src="/profile.png"
            ...
            <div className={`absolute inset-0 bg-gradient-to-t from-transparent to-transparent group-hover:from-purple-500/10 group-hover:to-transparent transition-colors duration-700`} />
          </div>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className={`w-full py-5 rounded-[30px] font-bold flex items-center justify-center gap-3 transition-all duration-300 border shadow-lg ${
            isLight 
      ...
      <div className="lg:col-span-7 flex flex-col gap-10 lg:gap-14 w-full">
        ...
        {/* Top Card: About Me Bio */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.2 }}
          className={`backdrop-blur-3xl border rounded-[60px] p-10 md:p-14 relative overflow-hidden transition-all duration-500 ${
            isLight 
            ? "bg-white/70 border-black/5 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)]" 
            : "bg-white/[0.01] border-white/[0.03] shadow-[0_0_50px_-20px_rgba(139,92,246,0.15)] hover:shadow-[0_0_60px_-15px_rgba(139,92,246,0.2)]"
          }`}
        >
          {/* Decorative Corner Element: Circular Text */}
          <div className="absolute top-[-30px] right-[-30px] md:top-[-50px] md:right-[-50px] pointer-events-none select-none overflow-visible">
            <motion.div
              animate={{ rotate: 360 }}
            ...
            >
              <svg viewBox="0 0 100 100" className="w-full h-full opacity-20">
                <path
                  ...
                  fill="none"
                />
                <text className={`text-[7px] font-mono uppercase tracking-[0.3em] font-bold ${
                  isLight ? "fill-black" : "fill-white"
                }`}>
                  <textPath xlinkHref="#circlePath">
                    Software Engineer • Based in Sri Lanka • AI Enthusiast • Specialized in Fullstack •
                  </textPath>
                </text>
              </svg>
            </motion.div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            ...
            <div className={`w-fit flex items-center gap-2 px-4 py-2 rounded-full border text-xs font-black uppercase tracking-[0.15em] ${
              isLight 
          ...
          <div className={`space-y-8 leading-loose text-lg md:text-xl font-light tracking-wide ${
            isLight ? "text-gray-600/90" : "text-gray-400"
          }`}>
            <p>
              I am a dedicated <strong className={`font-bold underline decoration-purple-500/60 decoration-4 underline-offset-8 ${
                isLight ? "text-gray-900" : "text-white/90"
              }`}>Fullstack Developer</strong> with a deep passion for <strong className={`font-bold ${
                isLight ? "text-gray-900" : "text-white/90"
              }`}>Artificial Intelligence</strong>. Based in Sri Lanka, I specialize in building scalable, intelligence-driven architectures.
            </p>
            <p>
              My expertise extends into <strong className={`font-bold ${
                isLight ? "text-gray-900" : "text-white/90"
        ...
        <motion.div
          ...
          className={`backdrop-blur-3xl border rounded-[60px] p-10 md:p-14 transition-all duration-500 ${
            isLight 
            ? "bg-white/70 border-black/5 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)]" 
            : "bg-white/[0.01] border-white/[0.03] shadow-[0_0_50px_-20px_rgba(139,92,246,0.15)] hover:shadow-[0_0_60px_-15px_rgba(139,92,246,0.2)]"
          }`}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {/* Journey Section */}
            <div>
              <h3 className={`text-base font-black mb-8 uppercase tracking-[0.25em] ${
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div
                    key={i}
                    className={`p-4 rounded-3xl border flex flex-col items-center gap-3 transition-all duration-300 group hover:-translate-y-1 ${
                      isLight 
                      ? "bg-white border-black/5 text-gray-900 hover:shadow-lg hover:border-purple-500/20" 
                      : "bg-white/[0.03] border-white/[0.05] text-white hover:bg-purple-500/[0.08] hover:border-purple-500/30"
                    }`}
                  >
                    <skill.icon size={24} className={`${
                      isLight ? "text-purple-600" : "text-purple-500"
